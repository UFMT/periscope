  ...
  return {
    ...
    _scoreAll: function(string, search, abbrev, searchIndex, abbrIndex, scores, allScores) {
      ...
      if (abbrIndex == abbrev.length) {
        ...
        this.fillArray(scores, trailScore, scores.length, string.length);
      ...
      while ((index = search.indexOf(c, searchIndex+1)) != -1) {
        ...
        if (this.isNewWord(string, index)) {
          scores[index-1] = 1;
          this.fillArray(scores, SCORE_BUFFER, scoreIndex+1, index-1);
        }
        else if (this.isUpperCase(string, index)) {
          this.fillArray(scores, SCORE_BUFFER, scoreIndex+1, index);
        }
        else {
          this.fillArray(scores, SCORE_NO_MATCH, scoreIndex+1, index);
        }
    ...
    isUpperCase : function(string,index) {
    	var c = string.charAt(index);
        return ("A" <= c && c <= "Z");
    },
    isNewWord : function(string,index) {
    	var c = string.charAt(index-1);
        return (WORD_SEPARATORS.indexOf(c) != -1);
    },
    fillArray : function(array, value, from, to) {
